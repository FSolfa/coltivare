...
class RelatedQuestionError(Exception):
    ...
    def __unicode__(self):
        return f"An unkown error occured: {self.error}." f" Please report it on {GITHUB_LINK}."
...
class GoogleSearchRequestFailedError(RelatedQuestionError):
    ...
    def __init__(self, url, keyword, status):
        self.url = url
        self.keyword = keyword
        self.status = status

    def __unicode__(self):
        return f"Failed to requests with status {self.status}: {self.url}/{self.keyword}"
