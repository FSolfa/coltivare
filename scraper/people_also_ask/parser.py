...
def extract_related_questions(document: BeautifulSoup) -> List[str]:
    div_questions = document.select(".related-question-pair div")
    if not div_questions:
        return []
    questions = [question.get("data-q") for question in div_questions if question.get("data-q")]
    return questions


def is_ol_but_not_a_menu(tag):
    return tag.name == "ol" and (not tag.has_attr("role") or (tag.has_attr("role") and tag["role"] != "menu"))
...
def has_youtube_link(tag):
    youtube_links = tag.findAll(lambda x: x.name == "a" and "youtube" in x.get("href", ""))
...
class MultipleCardsFeaturedSnippetTag(FeaturedSnippetParser):
    ...
    @property
    def heading(self):
        tag_heading = self.tag.find("h3", {"role": "heading"}) or self.tag.find("h2", {"role": "heading"})
        return tag_heading.text
...
def is_simple_featured_snippet_tag(tag):
    ...
    if not is_xpdopen:
        return False
    is_xpdopen_of_related_questions = tag.h2 is not None and tag.h2.text == "People also ask"
